...
if not os.path.exists('model_data.json'):
    model.create([
        Layer(
            weights=Initializers.random_weights((784, 10)),
            biases=Initializers.zeros_biases((784, 10)),
...
def test(epoch):
    model.predict(
        inputs=x_test[epoch].flatten() / 255,
        target=y_test[epoch],
        one_hot=[1 if x == y_test[epoch] else 0 for x in range(10)],
        output_rule=lambda x: np.argmax(x)
    )
model.reset_metrics()
